...
echo "==> Migration changelog" | tee -a "$LOG_FILE"

echo "Ensuring migration history table..." | tee -a "$LOG_FILE"
...
for file in "$REPO_ROOT"/db/migrations/V*.sql; do
  [ -e "$file" ] || continue
  ...
  run_psql_app -c \
    "INSERT INTO public.db_migration_history(script_name, checksum, kind, applied_by) VALUES ('${script_name}', '${checksum}', 'migration', current_user) ON CONFLICT (script_name) DO UPDATE SET checksum=EXCLUDED.checksum, kind=EXCLUDED.kind, applied_at=NOW(), applied_by=EXCLUDED.applied_by;" \
    | tee -a "$LOG_FILE"
done
